...
const FAQ = () => {
  ...
  // Send typed or voice-transcribed message
  const sendMessage = async (msg: string) => {
    ...
    // Play AI reply as voice
    const ttsResponse = await fetch("/api/tts", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ text: reply }),
    });
    const audioBlobReply = await ttsResponse.blob();
    const audioURL = URL.createObjectURL(audioBlobReply);
    const audio = new Audio(audioURL);
    audio.play();
  };

  // Record voice → Whisper → AI
  const startRecording = async () => {
    ...
    recorder.onstop = async () => {
      ...
      // 1. Speech-to-text
      const textResponse = await fetch("/api/whisper", { method: "POST", body: formData });
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <meta
        name="description"
        content="Frequently Asked Questions about Wisdom Empire. Get answers about proverbs, quotes, idioms, and how to use our cultural knowledge platform."
      />
        ...
        <div className="max-w-4xl mx-auto bg-gray-900">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-lg text-muted-foreground">
              Find answers to common questions about using Wisdom Empire
            </p>
          </div>

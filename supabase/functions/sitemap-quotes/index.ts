...
serve(async (req) => {
  ...
  try {
    ...
    const pageParam = url.pathname.match(/sitemap-quotes-(\d+)\.xml/)
...
`
    ...
    if (quotes) {
      for (const quote of quotes) {
        const lastmod = new Date(quote.created_at).toISOString()
        sitemap += `<url>
  <loc>${baseUrl}/quotes/${quote.id}</loc>
...
`
    ...
    console.error('Error generating quotes sitemap:', error)
    return new Response('Error generating quotes sitemap', {
